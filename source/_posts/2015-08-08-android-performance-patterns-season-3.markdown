...
[Android性能优化典范](https://www.youtube.com/playlist?list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE)的课程更新到第三季了，这次一共12个短视频课程，包括的内容大致有：新引入的更高效的ArrayMaps容器，使用Android系统特殊提供的容器避免自动装箱，避免使用枚举类型，注意onLowMemory与onTrimMemory的回调，避免内存泄漏，高效的位置更新操作，重复layout操作的性能影响，以及使用Batching，Prefetching优化网络请求，压缩传输数据等等使用技巧。下面是对这些课程的总结摘要，认知有限，理解偏差的地方请多多交流指正！

## 1)Fun with ArrayMaps
程序内存的管理是否合理得当对应用的性能有着很大的影响，而使用内存不得当很多时候都是因为对容器的滥用导致的。Android系统为自己编写了一些特殊的容器，例如SparseArray，今天要介绍的是一个新的容器，叫做**ArrayMaps**。

我们经常会使用到HashMap这个容器，它非常好用，但是它却很占用内存。下图演示了HashMap的简要工作原理：

![android_perf_3_arraymap_key_value](/images/android_perf_3_arraymap_key_value.png)

为了解决HashMap更占内存的弊端，Android提供了内存效率更高的[ArrayMap](https://android.googlesource.com/platform/frameworks/base.git/+/master/core/java/android/util/ArrayMap.java)。它内部使用两个数组进行工作，其中一个数组用来记录Hash过后的顺序列表，另外一个数组按序记录Key-Value值，如下图所示：
...
随着数组中的对象越来越多，查找访问单个对象的花费也会跟着增长，这里面体现的是内存占用与访问时间之间的权衡交换。
...
与HashMap相比，ArrayMap在遍历的时候也更加简单高效。

![android_perf_3_arraymap_list](/images/android_perf_3_arraymap_list.png)

前面说了很多ArrayMap的好处，那么也不是所有情况下都去使用ArrayMap，我们应该在满足下面几个条件的情况下才尽量使用ArrayMap：

* 对象个数的数量级最好是1000以内的
* 数据组织形式是以包含Map结构的

如果不满足上面的2个条件，建议还是不要使用ArrayMap了。我们需要学会在特定情形下做好权衡，选择相对更加高效的实现方式。

## 2)Beware Autoboxing
有时候性能问题也可能是因为那些所谓的小细节引起的，例如不经意的“自动装箱”。我们知道基础数据类型的大小：boolean(8 bits), int(32 bits), float(32 bits)，long(64 bits)，为了能够让这些基础数据类型在大多数Java容器中运作，会需要做一个autoboxing的操作，转换成Boolean，Integer，Float等对象，如下演示了循环操作的时候是否发生autoboxing行为的差异：
...
Autoboxing的行为经常发生在类似HashMap这样的容器里面，对HashMap的增删改查操作都发生了大量的Autoboxing的行为

![android_perf_3_autoboxing_hashmap](/images/android_perf_3_autoboxing_hashmap.png)

为了避免这些autoboxing带来的效率问题，Android才提供了一些如下的Map容器用来替代HashMap，不仅避免了autoboxing，还减少了内存占用：
...
Android官方强烈建议不要在Android里面使用到enum。
...
Android系统提供了一些回调来通知应用的内存使用情况，通常来说，当所有的background应用都被kill掉的时候，forground应用会收到**onLowMemory()**的回调。在这种情况下，需要尽快释放当前应用的非必须内存资源，以确保系统稳定运行。Android系统还提供了onTrimMemory()的回调，当系统内存达到某些条件的时候，所有正在运行的应用都会收到这个回调，同时在这个回调里面会传递以下的参数，代表不同的内存使用情况，下图介绍了各种不同的回调参数：
...
通常来说，View保持了Activity的引用，Activity同时还和其他内部对象也有可能保持引用关系。当屏幕发生旋转的时候，activity很容易发生泄漏，这样的话，里面的view也会发生泄漏。 为了避免这些严重的泄漏问题，需要特别留意以下的规则：

### 6.1)避免使用异步回调
异步回调被执行的时间不确定，很有可能发生在activity都已经被销毁之后，不仅仅很容易导致crash，还很容易发生内存泄露。

![android_perf_3_leak_asyncback](/images/android_perf_3_leak_asyncback.png)

### 6.2)避免使用Static对象
因为static的生命周期过长，使用不当很可能导致leak。

![android_perf_3_leak_static](/images/android_perf_3_leak_static.png)

### 6.3)避免把View添加到没有清除机制的容器里面
例如把view添加到WeekHashMap，因为它实际上使用的是Hard Reference，如果没有清除，可能会导致泄漏。
...
上面演示中有一个方法是**setInterval()**指的意思是每隔多长的时间获取一次位置更新，时间相隔越短，自然花费的电量就越多，但是时间相隔太长，又无法及时获取到更新的位置信息。其中存在的一个优化点是，我们可以通过判断返回的位置信息相同，而设置下次的更新间隔增加一倍，通过这种方式来减少电量的消耗，如下图所示：

![android_perf_3_location_reduce](/images/android_perf_3_location_reduce.png)

在位置请求的演示代码中还有一个方法是**setFastestInterval()**，因为整个系统很可能存在其他的应用也在请求位置更新，那些应用很有可能设置的更新间隔时间很短，这种情况下，我们就可以通过setFestestInterval的方法来过滤那些过于频繁的更新。
...
为了提供不同精度的定位需求，同时屏蔽实现请求的细节，Android提供了下面4种不同精度与耗电量的参数供应用来设置调用，应用只需要决定在适当的场景下使用对应的参数就好了，通过LocationRequest.setPriority()方法传递下面的参数就好了。
...
我们可以使用Systrace来跟踪特定的某段操作，如果发现了比较严重的丢帧现象，可能就是因为重复layout引起的。通常我们无法避免重复layout，在这种情况下，我们应该尽量保持View Hierarchy的层级比较浅，这样即使发生重复layout，也不会因为布局的层级比较深而增大了重复layout的倍数。另外还有一点需要特别注意，请避免在任何时候去调用**requestLayout()**方法，因为一旦调用了requestLayout，会导致该layout的所有父节点都发生重新layout的操作。

![android_perf_3_layout_request](/images/android_perf_3_layout_request.png)

## 9)Network Performance 101
在以前的课程里面，我们介绍过，网络请求的操作是比较耗电的操作，其中使用移动网络请求网络数据更是其中最耗电的一个操作。关于如何减少移动网络的耗电量，有两个重要的原则需要遵守：第一个是减少移动网络被激活的时间与次数，第二个是压缩传输数据。

### 9.1)减少移动网络被激活的时间与次数
通常来说，发生网络行为可以划分为如下图所示的三种类型，一个是用户主动触发的请求，另外一个是服务器的返回数据，最后一个是一些数据上报，行为上报，位置更新等等操作。

![android_perf_3_network_three_type](/images/android_perf_3_network_three_type.png)

我们永远都不应该使用轮询的方式去执行网络请求更新，这样不仅仅会造成严重的电量消耗，还会浪费网络流量，例如：

![android_perf_3_network_polling](/images/android_perf_3_network_polling.png)

Android官方推荐的是使用[Google Cloud Messaging](https://developers.google.com/cloud-messaging/)(在大陆，然并卵)，这个框架会帮忙把更新的数据推送给手机客户端，效率极高！在不能使用这套框架的情况下，我们应该遵循下面的规则来处理数据同步的问题：
...
最后，我们还可以使用**Prefetching**(预取)的技术来提前把一些数据拿到，避免后面再次发起网络请求，如下图所示：
...
在性能优化典范课程的第一季与第二季里面，我们都有提到过下面这样一个网络请求与电量消耗的示意图：

![android_perf_3_batching_networking](/images/android_perf_3_batching_networking.png)

发起网络请求与接收回传数据都比较耗电，在网络硬件模块被激活之后，会继续保持几十秒的电量消耗时间，直到没有新的网络行为之后，进入休眠状态。前面一个段落介绍了使用Batching的技术来捆绑网络请求，从而达到减少网络请求的频率。那么如何实现Batching技术呢？通常来说，我们可以会把那些发出的网络请求，先暂存到一个PendingQueue里面，等到条件合适的时候再触发Queue里面的网络请求。

![android_perf_3_batching_queue](/images/android_perf_3_batching_queue.png)

可是什么时候才算是条件合适了呢？最简单粗暴的，我们可以在Queue大小到10的时候触发任务，也可以是当手机开始充电，或者是手机连接到WiFi等等。手动去实现这些功能显得很复杂繁琐，Google为了解决这个问题，为我们提供了GCMNetworkManager来帮助实现那些功能，仅仅只需要调用API就OK了。

## 11)Optimizing Network Request Frequencies
前面的段落已经提到了应该减少网络请求的频率，是为了减少电量的消耗。可以使用Batching，Prefetching的技术来避免频繁的网络请求。Google提供了GCMNetworkManager来帮助开发者实现那些功能，通过提供的API，我们可以选择在接入WiFi，开始充电，等待移动网络被激活等条件下再次激活网络请求。

## 12)Effective Prefetching
假设我们有这样的一个场景，刚开始网络请求了一张图片，隔了10秒需要请求另外一张图片，再隔6秒会请求第三张图片，如下图所示：

![android_perf_3_prefetching](/images/android_perf_3_prefetching.png)

预先请求会使用到网络资源，避免频繁的触发网络请求，这样能够显著的减少电量的消耗。可是预先获取多少数据量是很有讲究的，因为如果预取偏少，就起不到作用，可是如果预取多了，就会造成资源浪费。

![android_perf_3_prefetching_over](/images/android_perf_3_prefetching_over.png)

预取可以是按照不同网络状态下事先获取不同大小的数据包来作为依据，也可以是按照图片数量或者是之后几分钟内可能用到的数据作为阀值。我们需要根据特定的场景，不同的网络情况设计合适的方案。
